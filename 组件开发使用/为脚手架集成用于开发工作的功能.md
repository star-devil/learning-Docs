...
>
> 💡 如果你使用了 Tailwind CSS v4，它已内置 autoprefixer，无需额外安装。脚手架 JS 模板已移除 autoprefixer，TS 模板中也不包含——因为 Tailwind v4 会自动处理前缀。
...
3. 创建`tailwind.css`文件，添加 Tailwind 的基础指令，并在main.ts中引入
...
import { createApp } from 'vue'
import App from './App.vue'
import './styles/tailwind.css'; // 包含 Tailwind 指令的 CSS 文件 
...
>
>   之前使用的是`vite-plugin-imagemin`插件，star较多，但是`gifsicle`这个依赖在国区总是遇到下载问题，所以后面换成了`vite-plugin-minipic`。
>
>   > ⚠️ **脚手架模板已移除图片压缩插件**。`vite-plugin-minipic` 依赖 `sharp` 原生二进制包，在 Windows 上安装时经常因为缺少 Visual Studio 构建工具而失败，影响脚手架开箱即用的体验。如果需要图片压缩功能，可以在项目中手动安装并参考下面的配置。有成熟的图片处理流程时也可以交给 CI/CD 或 CDN 处理。
>
>   `vite-plugin-minipic` 是基于 `sharp` 实现的，安装时可能遇到平台相关问题，作者给出了解决方案：https://github.com/60late/vite-plugin-minipic/issues/2
...
export defaul defineConfig({
  plugins:[
    ...
    minipic({     // 自定义用法
      ...
      /** ⚠️注意：当 sharp 解析 GIF 时，它会尝试把所有帧解码合并为一个大图（称为“pages stacking”），尺寸变成：width x height x pages（帧数）,当像素总量（width × height × frames）超过 sharp 的 limitInputPixels 限制（默认 268402689），就会报错。当 sharp 处理较大 gif 时需要配合插件提供limitInputPixels选项，默认为true，如果超出sharp的最大限制，那么就跳过这张gif的压缩。如果该值设置为false，那么就允许用户强制压缩当前gif图片，但是如果机器内存不够会容易造成OOM。另外，我用了一张 6.26M的 gif测试，压缩率只有1.15%（用了467.83Kb的小图测试，压缩率为6.46%，图片越大压缩率越低）效果不是很好；but！！如果选择跳过压缩...emm，这个大 gif打包之后是 0kb，应该是编码错误了，不知道怎么解决。所以系统内还是不要用大的 gif 图😅*/
      // 其他参数请查阅文档
    }),
  ]
})
...
![压缩前图片大小](../images/cli/压缩前图片大小.png)

![压缩后图片大小](../images/cli/压缩后图片大小.png)
...
pinia.use(
	createPersistedState({
		...
		// 该配置改为 false 时，Store 需要显式设置 persist: true 才会持久化。
		// 脚手架默认使用 auto: false，避免意外存储敏感数据。
		auto: false
	})
);
...
// 直接导出配置好的 NProgress 实例
// 使用处：NProgress.start() / NProgress.done()
export default NProgress
```

> 脚手架模板在路由守卫（`src/router/guards.ts`）中集成了 NProgress，路由切换时自动显示和隐藏进度条，无需在页面中手动调用。
```
...
> 如果数据在缓存中是明文的话，使用者可以任意篡改，并影响系统运行，会存在一定的安全隐患。

1. 使用 `crypto-js` 进行 AES 加密
   ...
   - 实现加密、解密方法
     ...
     class AESEncryptor {
       ...
       static generateAesKey(): string {
         return import.meta.env.VITE_SECRET_KEY; // 密钥请自行生成并保存。
       }
     ...
     /** RSA加密对象: 需要接口返回公钥，使用公钥加密，解密过程放在后端，可以使用jsencrypt库实现加密 */
